        ...
        <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <img
                      src={
                        item.images?.[0]?.url
                          ...
                          : "/placeholder.png" // fallback image if no image
                      }
                      alt={item.name}
                    />
                  </div>
              ...
              <div className="mt-10">
                ...
                {orders.map((order, idx) => (
                  <div
                    ...
                    className="bg-gray-900/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-4 mb-4"
                  >
          ...
          <div className="lg:col-span-1">
            <div className="bg-gradient-to-br from-gray-800/50 to-black/50 backdrop-blur-sm border border-gray-700/50 rounded-2xl lg:rounded-3xl p-4 sm:p-6 lg:p-8 sticky top-8">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-4 lg:mb-6">
                {t("cart.orderSummary")}
              </h2>
              ...
              <div className="space-y-2 sm:space-y-3 lg:space-y-4 mb-4 lg:mb-6">
                {cart?.map((item) => (
                  <div
                    key={item.productId}
                    className="flex justify-between text-gray-300 text-xs sm:text-sm lg:text-base"
                  >
                    <span className="flex-1 pr-2">
                      {item.name} × {item.quantity}
                    </span>
                    <span className="font-medium">
                      {formatPrice((item.sellingPrice || 0) * item.quantity)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              {/* Add More Items button (back to products) */}
              <Link
                to="/products"
                className="w-full flex items-center justify-center gap-2 mb-3 border border-amber-500 text-amber-400 py-2 rounded-xl font-semibold text-sm sm:text-base hover:bg-amber-500/10 transition-colors duration-200"
              >
                <Plus size={16} />
